<template>
  <dashboard_layout page_title="One Health Sectors">

    <div class="mb-3 flex flex-row justify-end">      
      <router-link
          :to="{name:'CreateOneHealthSector', params:{}}"
      ...
      >
        ...
        <span class="px-1">Add Sector</span>
      </router-link>
    </div>
    ...
    <data_table v-bind="$attrs">
      <template v-slot="{item}">        
        ...
        <td class="px-4 py-3 text-sm flex flex-row space-x-1">   
          <router-link
              :to="{name:'UpdateOneHealthSector', params:{sectorId:item.id, sectorName: item.name}}"
